...
public class IETFTest extends BaseTest {
    ...
    public void pyeccBector() {
        ...
        short[] msg = {3, 1, 4, 1, 5, 9};
        PrivateKey skobj = PrivateKey.fromBytes(Util.hexToBytes(sk));
        G2Element sig = new BasicSchemeMPL().sign(skobj, new Uint8Vector(msg));
        Uint8Vector sig1 = new Uint8Vector(Util.byteArrayToShortArray(Util.hexToBytes(sig1BasicHex)));
        assertTrue(DASHJBLS.objectEquals(sig, G2Element.fromByteVector(sig1)));
    }
}
