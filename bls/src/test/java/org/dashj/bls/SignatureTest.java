...
public class SignatureTest extends BaseTest {
    ...
    public void shouldNotVerifyAggregateWithSameMessageUnderBasicScheme() {
        ...
        assertFalse(new BasicSchemeMPL().aggregateVerify(new G1ElementVector(new G1Element[]{pk1, pk2}), new ByteVectorList(new byte[][]{message, message}), aggSig));
    }
    ...
    public void shouldVerifyAggregateWithSameMessageUnderAugSchemePopScheme() {
        ...
        G2Element aggSigAug = new AugSchemeMPL().aggregate(new G2ElementVector(new G2Element[]{sig1Aug, sig2Aug}));
        assertTrue(new AugSchemeMPL().aggregateVerify(new G1ElementVector(new G1Element[]{pk1, pk2}), new ByteVectorList(new byte[][]{message, message}), aggSigAug));

        G2Element sig1Pop = new PopSchemeMPL().sign(sk1, message);
        G2Element sig2Pop = new PopSchemeMPL().sign(sk2, message);
        G2Element aggSigPop = new PopSchemeMPL().aggregate(new G2ElementVector(new G2Element[]{sig1Pop, sig2Pop}));
        assertTrue(new PopSchemeMPL().aggregateVerify(new G1ElementVector(new G1Element[]{pk1, pk2}), new ByteVectorList(new byte[][]{message, message}), aggSigPop));
    }
    ...
    public void shouldAugAggregateManyG2ElementsDiffMessage() {
        ...
        for (int i = 0; i < 80; i++) {
            ...
            sigs.add(sig);
            ms.add(new Uint8Vector(Util.byteArrayToShortArray(message)));
        }
    ...
    @Test public void aggregateVerificationOfZeroItemsWithInfinity() {
