...
public class HKDF256 {
  ...
  public synchronized void delete() {
    if (swigCPtr != 0) {
      if (swigCMemOwn) {
        swigCMemOwn = false;
        ZIPJBLSJNI.delete_HKDF256(swigCPtr);
      }
      swigCPtr = 0;
    }
  }
  ...
  public static void extract(byte[] prk_output, byte[] salt, long saltLen, byte[] ikm, long ikm_len) {
    ZIPJBLSJNI.HKDF256_extract(prk_output, salt, saltLen, ikm, ikm_len);
  }

  public static void expand(byte[] okm, long L, byte[] prk, byte[] info, long infoLen) {
    ZIPJBLSJNI.HKDF256_expand(okm, L, prk, info, infoLen);
  }
  ...
  public static void extractExpand(byte[] output, long outputLen, byte[] key, long keyLen, byte[] salt, long saltLen, byte[] info, long infoLen) {
    ZIPJBLSJNI.HKDF256_extractExpand(output, outputLen, key, keyLen, salt, saltLen, info, infoLen);
  }

  public HKDF256() {
    this(ZIPJBLSJNI.new_HKDF256(), true);
  }

  public final static short HASH_LEN = ZIPJBLSJNI.HKDF256_HASH_LEN_get();
}
